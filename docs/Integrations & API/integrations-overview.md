...
- **Remove a single capability** by clicking the **×** on its badge — for example, drop **Calendar** from a Google account while keeping **Meet**. Removing the last capability disconnects the account entirely.
