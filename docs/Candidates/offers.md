...
When a candidate makes it to the end of your pipeline, Hivemind can generate their offer letter, route it through your team for approval, and collect a legally binding e-signature — no printing, no third-party signing tool. This guide covers setting up the **Offer** step, the internal signing chain, and what candidates see.
...
When it's an approver's turn, a task lands in their Hivemind inbox: a live PDF preview of the letter on the left, and **Your fields** plus a **Sign** section on the right. They type or draw a signature, tick the consent boxes, and click **Sign and pass to {next approver}** — or, if they're last, **Sign and send to {candidate}**. Approvers can also **Send back** an offer to a previous signer with a note, **Decline** it, or **Withdraw** it entirely.
...
The candidate receives "Your offer from {company} is ready to sign" with a **Review and sign** button. They first verify their identity with a 6-digit code emailed to them, then land on the signing page: the offer letter on the left, **Add your signature** on the right, with **Type** and **Draw** options. After ticking the consent boxes they click **Sign & finish** — or choose **Request changes** to open a negotiation, or **Decline**.
