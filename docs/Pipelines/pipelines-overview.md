...
To go live, open the pipeline and choose **Activate** from its **Manage** menu. Hivemind first checks that every step is fully configured and that the job details (title, description, location, workplace type, job type) are filled in — you'll see **Cannot Activate** or **Incomplete Pipeline** messages pointing at anything missing. On success: *"Your pipeline is now live and accepting applications."*
